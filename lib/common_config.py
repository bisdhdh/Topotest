...
def start_deamons_and_routers(tgen, CWD):
    """
    It will create temporary folders and files to start 
    deamons and routers.
    ...
    * `CWD` : Caller's current working directory
    ...
    # Starting routers, once deamons started
    logger.info("Starting all routers once topology is created")
    tgen.start_router()

def remove_temp_files(tgen, CWD):
    """
    It will remove temporary dirs and files.
...
def add_static_route_for_loopback_interfaces(ADDR_TYPE, curRouter, topo, frrcfg):
    ...
    * `frrcfg` : frr config file
    ...
    for bgp_neighbor in bgp_neighbors.keys():
        if bgp_neighbors[bgp_neighbor]['peer']['source'] == 'lo':
            ip_addr = topo['routers'][bgp_neighbor]['lo'][ADDR_TYPE]
            destRouterLink = bgp_neighbors[bgp_neighbor]['peer']['link']

...
def create_bgp_cfg(router, topo):
    ...
    * `topo` : json file data
	...
	if "enabled" in topo['routers']['{}'.format(router)][key]:
        ...
        bgp = Bgp(local_as, graceful_restart, ecmp)
...
class Neighbor:
    ...
    def __init__(self, afi, ip_address, remote_as, keep_alive_time=None,
                 ...
                 max_hop_limit = 0, graceful_restart=None):
        self.afi = afi
...
class Bgp:
    ...
    def add_neighbor(self, afi, ip_address, remote_as, keep_alive_time=None,
                     hold_down_time=None, password=None, update_source=None,
                     max_hop_limit=None, graceful_restart=None):
...
def _print_bgp_neighbors_cfg(bgp_cfg, neighbor):
    ...
    if neighbor.update_source != None:
        bgp_cfg.bgp_neighbors.write(
            neigh_cxt + 'update-source_link ' + neighbor.update_source + ' \n')
    keep_alive = '60'
    hold_down = '180'
...
def configure_graceful_restart(ADDR_TYPE, input_dict, tgen, CWD, topo):
    """
    configure_graceful_restart
    ...
    * `topo`  : json file data
    ...
    logger.info("Entering lib API: configure_graceful_restart()")
    ...
    try:
        for router in input_dict.keys():
            ...
            for neighbor in neighbors.keys():
                ...
                list_of_neighbors = bgp_cfg[router].routing_pb.bgp_config.neighbors
                for n in list_of_neighbors:
                    if IpAddressMsg_to_str(n.ip_address) == neighbor_ip:
                        n.graceful_restart = str(graceful_restart)
		

            if "gracefulrestart" in input_dict[router]["bgp"] :  
                global_graceful_restart = input_dict[router]["bgp"]['gracefulrestart']
                bgp_cfg[router].routing_pb.bgp_config.graceful_restart = global_graceful_restart
...
## Verification API for graceful restart
def verify_graceful_restart(ADDR_TYPE, input_dict, tgen, topo, dut):
    """
    This API is to verify verify_graceful_restart for any router.
    ...
    * `topo`: input json file data
    """
    logger.info("Entering lib API: verify_graceful_restart()")
    ...
    for router, rnode in tgen.routers().iteritems():
        ...
        if show_bgp_graceful_json_out["remoteGrMode"] == rmode :
            ...
            logger.error('remoteGrMode is not correct {}'.format(show_bgp_graceful_json_out["remoteGrMode"]))
            return False


    logger.info("Exiting lib API: verify_graceful_restart()")
    return True



## Verification API for r-bit
def verify_r_bit(ADDR_TYPE, input_dict, tgen, topo, dut):
    """
    This API is to verify verify_r_bit for any router.
    ...
    * `topo`: input json file data
    """
    logger.info("Entering lib API: verify_r_bit()")
...
def verify_GR_status(ADDR_TYPE, tgen, input_dict, topo):
    """
...
def verify_bgp_rib(ADDR_TYPE, dut, tgen, input_dict, next_hop = None, protocol = None):
    """
    ...
    * `protocol`[optional]: protocol, default = None
    ...
    logger.info("Entering lib API: verify_bgp_rib()")
    ...
    for routerInput in input_dict.keys():
        for router, rnode in router_list.iteritems():
            ...
            sleep(2)
            logger.info('Checking router {} RIB:'.format(dut))
            rib_routes_json = rnode.vtysh_cmd(command, isjson=True)
	    print(rib_routes_json)
            ...
            if 'static_routes' in input_dict[routerInput]:
                ...
                for static_route in static_routes:
                    ...
                    for st_rt in ip_list:
                        ...
                        if st_rt in rib_routes_json["routes"]:
                            ...
                            if next_hop != None:
                                if rib_routes_json["routes"][st_rt][0]['nexthops'][0]['ip'] \
                                        == next_hop:
                                    nh_found = True
                                else:
                ...
                if nh_found:
                    logger.info("Found next_hop {} for all {} routes in RIB of"
                                " router {}\n".format(next_hop, protocol, router))
    ...
    logger.info("Exiting lib API: verify_bgp_rib()")
    return True
